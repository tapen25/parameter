...
class MuseMorphose(nn.Module):
  def __init__(self, enc_n_layer, enc_n_head, enc_d_model, enc_d_ff, 
    ...
    d_tempo_emb=64, d_density_emb=64, d_velocity_emb=64,
    n_tempo_cls=8, n_density_cls=8, n_velocity_cls=9,
    ...
    if use_attr_cls:
      self.decoder = VAETransformerDecoder(
        dec_n_layer, dec_n_head, dec_d_model, dec_d_ff, d_vae_latent + d_tempo_emb + d_density_emb + d_velocity_emb,
    ...
    if use_attr_cls:
      self.tempo_attr_emb = TokenEmbedding(n_tempo_cls, d_tempo_emb, d_tempo_emb)
      self.density_attr_emb = TokenEmbedding(n_density_cls, d_density_emb, d_density_emb)
      # IDs 0..7: velocity levels; ID 8: undefined (no note onsets).
      # This is a learned category, not a padding token or a loss mask.
      self.velocity_attr_emb = TokenEmbedding(n_velocity_cls, d_velocity_emb, d_velocity_emb)
  ...
  def combine_attributes(self, dec_seg_emb, tempo_cls, density_cls, velocity_cls):
    if not self.use_attr_cls:
      return dec_seg_emb
    if any(c is None for c in (tempo_cls, density_cls, velocity_cls)):
      raise ValueError('tempo_cls, density_cls and velocity_cls are required')
    dec_tempo_emb = self.tempo_attr_emb(tempo_cls)
    dec_density_emb = self.density_attr_emb(density_cls)
    dec_velocity_emb = self.velocity_attr_emb(velocity_cls)
    return torch.cat(
      [dec_seg_emb, dec_tempo_emb, dec_density_emb, dec_velocity_emb], dim=-1
    )

  def generate(self, inp, dec_seg_emb, tempo_cls=None, density_cls=None, velocity_cls=None, keep_last_only=True):
    token_emb = self.token_emb(inp)
    dec_inp = self.emb_dropout(token_emb) + self.pe(inp.size(0))

    dec_seg_emb_cat = self.combine_attributes(
      dec_seg_emb, tempo_cls, density_cls, velocity_cls
    )
  ...
  def forward(self, enc_inp, dec_inp, dec_inp_bar_pos, tempo_cls=None, density_cls=None, velocity_cls=None, padding_mask=None):
    ...
    # [shape of tempo_cls, density_cls, velocity_cls] same as above 
    ...
    _, mu, logvar = self.encoder(enc_inp, padding_mask=padding_mask)
    vae_latent = self.reparameterize(mu, logvar, use_sampling=self.training)
  ...
  def compute_loss(self, mu, logvar, beta, fb_lambda, dec_logits, dec_tgt, bar_mask=None):
    ...
    kl_terms = -0.5 * (1 + logvar - mu ** 2 - logvar.exp())
    if bar_mask is not None:
      kl_terms = kl_terms[bar_mask.reshape(-1)]
    kl_raw = kl_terms.mean(dim=0)
